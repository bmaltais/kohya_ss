...
class DreamBoothOrFineTuningDataset(torch.utils.data.Dataset):
  def __init__(self, fine_tuning, train_img_path_captions, reg_img_path_captions, tokenizer, resolution, flip_aug, color_aug, face_crop_aug_range, random_crop, shuffle_caption, debug_dataset) -> None:
    super().__init__()
  ...
  def __getitem__(self, index_arg):
    ...
    example['caption_ids'] = self.tokenizer(caption, padding="do_not_pad", truncation=True,
...
class LatentsCachedDataset(torch.utils.data.Dataset):
  def __init__(self, latents_cache, examples):
  ...
  def __getitem__(self, index):
    ...
    return {'latents': self.latents_cache[example['image_path']], **example}
...
def train(args):
  ...
  train_dataset = DreamBoothOrFineTuningDataset(fine_tuning, train_img_path_captions,
                                                reg_img_path_captions, tokenizer, resolution, args.flip_aug, args.color_aug, face_crop_aug_range, args.random_crop, args.shuffle_caption, args.debug_dataset)
  ...
  if args.debug_dataset:
    ...
    for example in train_dataset:
      ...
      print(f'caption: "{example["caption"]}", reg: {example["reg"]}')
      cv2.imshow("img", im)
      k = cv2.waitKey()
    ...
    for i in tqdm(range(len(train_dataset))):
      ...
      if image_path not in latents_cache:
        with torch.no_grad():
          ...
          latents = vae.encode(pixel_values).latent_dist.sample().to("cpu")
          latents_cache[image_path] = latents.squeeze(0)
      del example['image']
      examples.append(example)

    train_dataset = LatentsCachedDataset(latents_cache, examples)
    del vae
  ...
  # dataloaderを準備する
  disable_padding = args.no_token_padding

  def collate_fn(examples):
    input_ids = [e['caption_ids'] for e in examples]
    regs = [e['reg'] for e in examples]                 # waitを変えたい
    ...
    # padしてTensor変換
    if disable_padding:
      # paddingしない：padding==Trueはバッチの中の最大長に合わせるが、バッチサイズ1なので（やはりバグでは……？）
    ...
    loss_weights = [(1.0 if not reg else args.prior_loss_weight) for reg in regs]
