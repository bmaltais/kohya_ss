...
def load_images(
    ...
    def empty_return():
        return [loaded_images_dir, loaded_control_images_dir, page, max_page]

    # Check for images_dir
    if not target_images_dir:
        msgbox("Target image folder is missing...")
        return empty_return()

    if not os.path.exists(target_images_dir):
        msgbox("Target image folder does not exist...")
        return empty_return()

    if not control_images_dir:
        msgbox("Control image folder is missing...")
        return empty_return()

    if not os.path.exists(control_images_dir):
        msgbox("Control image folder does not exist...")
        return empty_return()
    ...
    # Load Images
    target_images_list = os.listdir(target_images_dir)
    control_images_list = os.listdir(control_images_dir)

    target_image_files = {
        f for f in target_images_list if f.lower().endswith(IMAGE_EXTENSIONS)
    }
    control_image_files = {
        f for f in control_images_list if f.lower().endswith(IMAGE_EXTENSIONS)
    }
    ...
    return [
        ...
        control_images_dir,
        1,
        ceil(total_images / IMAGES_TO_SHOW),
    ]
...
def update_images(
    ...
    image_files = list(target_image_files.intersection(control_image_files))
...
# Gradio UI
def gradio_flux_manual_caption_gui_tab(headless=False, default_images_dir=None):
    ...
    with gr.Tab("Flux Manual Captioning"):
        gr.Markdown(
            "This utility allows quick captioning and tagging of images for Flux.1 Kontext."
        )
        ...
        with gr.Group(), gr.Row():
            target_images_dir = gr.Dropdown(
                label="Target image folder (containing the images to caption)",
            ...
            create_refresh_button(
                target_images_dir,
                lambda: None,
                lambda: {"choices": list_dirs_wrapper(current_images_dir, "target")},
                "open_folder_small",
            )
            ...
            control_images_dir = gr.Dropdown(
                label="Control image folder (containing the control images)",
                choices=[""]
                + list_dirs_wrapper(default_images_dir, "control"),
                value="",
                interactive=True,
                allow_custom_value=True,
            )
            create_refresh_button(
                ...
                lambda: None,
                lambda: {
                    "choices": list_dirs_wrapper(current_control_images_dir, "control")
                },
                "open_folder_small",
            )
            control_folder_button = gr.Button(
                "📂",
                elem_id="open_folder_small",
                elem_classes=["tool"],
                visible=(not headless),
            )
            control_folder_button.click(
                get_folder_path,
                outputs=control_images_dir,
                show_progress=False,
            )
            ...
            target_images_dir.change(
                fn=lambda path: gr.Dropdown(
                    choices=[""] + list_dirs_wrapper(path, "target")
                ),
            ...
            control_images_dir.change(
                fn=lambda path: gr.Dropdown(
                    choices=[""] + list_dirs_wrapper(path, "control")
                ),
        ...
        listener_kwargs = {
            "fn": lambda p, i, j: f"{p}-{i}-{j}",
            "inputs": [page, loaded_images_dir, loaded_control_images_dir],
