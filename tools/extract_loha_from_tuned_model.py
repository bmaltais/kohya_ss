...
def initialize_loha_parameters(
    ...
    is_continuation_attempt: bool, # True if existing_loha_layer_parameters provided to optimize_loha_for_layer
    existing_params_to_load: dict | None = None, # From original --continue_training_from_loha or previous opt result
    warm_start_status: str | None = None, # 'applied', 'skipped_cannot_warm_start', etc.
    prev_rank_for_warm_start: int | None = None # Rank of prev_best_params_for_warm_start
):
    """
    Initializes or loads LoHA parameter tensors (hada_w1_a, hada_w1_b, hada_w2_a, hada_w2_b).
    Handles loading from existing_params_to_load (e.g. from --continue_training_from_loha or initial call to optimize_loha_for_layer)
    or warm-starting from a previous rank's successful optimization (prev_best_params_for_warm_start).
    If neither, initializes with Kaiming/Normal.

    Returns:
        Tuple[nn.Parameter, nn.Parameter, nn.Parameter, nn.Parameter]:
            hada_w1_a_p, hada_w1_b_p, hada_w2_a_p, hada_w2_b_p
    """
    ...
    with torch.no_grad():
        # Case 1: Initial attempt for this layer, and we have existing LoHA parameters to load
        # (either from --continue_training_from_loha or if optimize_loha_for_layer was called with them directly)
        if is_continuation_attempt and attempt_idx == 0 and existing_params_to_load:
            try:
                # Ensure the rank matches before trying to load
                ...
                # else: Ranks don't match, will fall through to Kaiming/Normal or warm-start if applicable
            except Exception as e:
                if args_global.verbose_layer_debug: # Check args_global as it might not be set if called outside main flow
                    tqdm.write(f"    {layer_name}: Error loading initial existing params: {e}. Falling back.")
                pass # Fall through to other initialization methods

        # Case 2: Warm-starting from a previous, lower-rank optimization attempt in the same layer
        # `warm_start_status` is determined in `optimize_loha_for_layer` before calling this.
        # `existing_params_to_load` in this context is the `best_result_so_far` from `optimize_loha_for_layer`
        if not initialized_from_external_or_warm_start and warm_start_status == 'applied' and existing_params_to_load and prev_rank_for_warm_start is not None:
            # `existing_params_to_load` here refers to `best_result_so_far`'s tensors from previous rank
            prev_params_cpu = {
                'hada_w1_a': existing_params_to_load['hada_w1_a'],
                'hada_w1_b': existing_params_to_load['hada_w1_b'],
                'hada_w2_a': existing_params_to_load['hada_w2_a'],
                'hada_w2_b': existing_params_to_load['hada_w2_b'],
            }
            # Copy data from the lower rank
            ...
            # Initialize the new parts of the tensors
            if current_rank > prev_rank_for_warm_start:
                for p_slice in [
                    hada_w1_a_p.data[:, prev_rank_for_warm_start:],
                    hada_w2_a_p.data[:, prev_rank_for_warm_start:]
                ]:
                    nn.init.kaiming_uniform_(p_slice, a=math.sqrt(5))
                for p_slice in [
                    hada_w1_b_p.data[prev_rank_for_warm_start:, :],
                    hada_w2_b_p.data[prev_rank_for_warm_start:, :]
                ]:
                    nn.init.normal_(p_slice, std=0.02)
            initialized_from_external_or_warm_start = True
            # Logging for warm start is done in optimize_loha_for_layer via RANK_INCREASED_INFO

        # Case 3: Default initialization (Kaiming/Normal)
        if not initialized_from_external_or_warm_start:
            ...
            for p in [hada_w1_a_p, hada_w2_a_p]:
                nn.init.kaiming_uniform_(p.data, a=math.sqrt(5))
            for p in [hada_w1_b_p, hada_w2_b_p]:
...
def optimize_loha_for_layer(
    ...
    is_conv: bool = True, verbose_layer_debug: bool = False, # args_global.verbose_layer_debug
    max_rank_retries: int = 0, # args_global.max_rank_retries (potentially adjusted)
    rank_increase_factor: float = 1.25, # args_global.rank_increase_factor
    existing_loha_layer_parameters: dict | None = None # From main() if --continue_training_from_loha
):
    delta_W_target = delta_W_target.to(device, dtype=dtype)
    # True if optimize_loha_for_layer is called with pre-existing weights for the very first attempt
    is_initial_call_with_existing_params = existing_loha_layer_parameters is not None

    best_result_so_far = { # Stores results for the best rank attempt *within this call* to optimize_loha_for_layer
    ...
    for attempt_idx in range(max_rank_retries + 1): # attempt_idx is 0 for the first try (initial_rank_for_layer)
        ...
        current_warm_start_status = None # For logging with RANK_INCREASED_INFO
        params_for_initialization = None
        prev_rank_for_warm_start_log = None

        if attempt_idx > 0: # This is a rank retry
            log_layer_optimization_event(LogType.RANK_RETRY_STARTING, layer_name,
                                         prev_rank=rank_base_for_next_increase, # rank of the attempt that just finished
            ...
            # Recalculate alpha based on the original ratio provided to this function call
            ...
            prev_rank_for_warm_start_log = best_result_so_far.get('final_rank_used') # Rank that produced best_result_so_far
            if 'hada_w1_a' in best_result_so_far and not args_global.no_warm_start:
                if prev_rank_for_warm_start_log < current_rank_for_this_attempt:
                    current_warm_start_status = 'applied'
                    params_for_initialization = best_result_so_far # Pass the whole dict containing tensors
                else:
                    current_warm_start_status = 'skipped_cannot_warm_start'
            elif args_global.no_warm_start and 'hada_w1_a' in best_result_so_far:
                 current_warm_start_status = 'skipped_no_warm_start_arg'
            elif 'hada_w1_a' not in best_result_so_far: # Should not happen if attempt_idx > 0 and a previous attempt ran
                 current_warm_start_status = 'no_prior_params_for_warm_start'
            ...
            log_layer_optimization_event(LogType.RANK_INCREASED_INFO, layer_name,
                                         ...
                                         warm_start_status=current_warm_start_status,
                                         prev_rank_for_warm_start=prev_rank_for_warm_start_log)
        elif is_initial_call_with_existing_params: # First attempt (attempt_idx == 0) AND we got params from main
            params_for_initialization = existing_loha_layer_parameters

        k_ops = k_h * k_w if is_conv else 1

        # --- CALL NEW INITIALIZATION FUNCTION ---
        hada_w1_a_p, hada_w1_b_p, hada_w2_a_p, hada_w2_b_p = initialize_loha_parameters(
            out_dim=out_dim, current_rank=current_rank_for_this_attempt,
            in_dim_effective_k_ops=(in_dim_effective * k_ops),
            device=device, dtype=dtype, layer_name=layer_name, attempt_idx=attempt_idx,
            is_continuation_attempt=(attempt_idx == 0 and is_initial_call_with_existing_params), # True if it's the first try AND initial params were given
            existing_params_to_load=params_for_initialization, # This is existing_loha_layer_parameters on 1st attempt, or best_result_so_far on retries
            warm_start_status=current_warm_start_status, # Relevant for attempt_idx > 0
            prev_rank_for_warm_start=prev_rank_for_warm_start_log if current_warm_start_status == 'applied' else None
        )
        # --- END CALL ---
        ...
        for i in iter_pbar:
            # ... (rest of the optimization loop remains the same) ...
        ...
        if current_attempt_iterations_done < max_iterations and not any([current_attempt_insufficient_progress, current_attempt_stopped_by_projection, current_attempt_stopped_early_by_loss]):
            if not is_last_rank_attempt: # Only log this if we are actually going to retry
        ...
        if is_last_rank_attempt:
             ...
             break # Explicitly break after last attempt summary

        # If we are going to retry (i.e., not stopped by loss and not the last attempt)
        if not current_attempt_stopped_early_by_loss : 
            reason_kwargs = {'attempt': attempt_idx + 1, 'rank': current_rank_for_this_attempt, 'is_last_rank_attempt': is_last_rank_attempt} # is_last_rank_attempt will be False here
            ...
            elif current_attempt_iterations_done >= max_iterations: # Max iterations hit, target not met (or no target)
            ...
            # Only log ATTEMPT_ENDED_WILL_RETRY if a reason was actually determined for stopping this attempt *and we are not on the last attempt*
            if 'reason_type' in reason_kwargs : 
                log_layer_optimization_event(LogType.ATTEMPT_ENDED_WILL_RETRY, layer_name, **reason_kwargs)
            # If no specific stop reason but also not target_loss_met, and not last attempt, it implies it finished early without a stop flag
            # This case is covered by ATTEMPT_EARLY_FINISH_NO_STOP_FLAG already if it didn't hit max_iterations.
            # If it did hit max_iterations, it's covered by the reason_kwargs above.
    
    if 'hada_w1_a' not in best_result_so_far: # If no successful attempt produced parameters
    ...
    # Ensure all expected keys are present in the returned dictionary
    for key, default_val in [('stopped_early_by_loss', False), ('stopped_by_insufficient_progress', False), 
                             ...
                             ('final_rank_used', initial_rank_for_layer)]: # final_rank_used should always be set by a successful attempt
...
def handle_resume_or_continue_loha(
    ...
    """
    Handles loading state for continuing LoHA training or resuming a standard extraction.
    Modifies the passed-in dictionaries and sets in place.
    """
    if current_args.continue_training_from_loha:
        ...
        try:
            ...
            extracted_sd_ref.update(loaded_sd) # Load all tensors into the main SD for saving later
        ...
        if resume_file:
            ...
            try:
                ...
                if not completed_in_file and loaded_sd_resume: # Fallback if metadata missing but file exists
                ...
                if completed_in_file:
                    for k, v in loaded_sd_resume.items():
                        # Load tensors for completed modules or bias terms
                        module_prefix_of_key = ".".join(k.split('.')[:-1])
                        if module_prefix_of_key in completed_in_file or k.endswith(".bias"): # Bias keys might not be in completed_in_file directly
                            extracted_sd_ref[k] = v
                            res_tensor_count += 1
                    prev_completed_prefixes_ref.update(completed_in_file)
                    all_completed_prefixes_ref.update(completed_in_file) # Crucial for skipping already processed layers
                    print(f"  Loaded {len(prev_completed_prefixes_ref)} module prefixes, {res_tensor_count} tensors for resume.")
                elif loaded_sd_resume: # If no completed_in_file list, but a resume file exists, load all (e.g. old format)
                    extracted_sd_ref.update(loaded_sd_resume)
                    # Try to infer completed prefixes if possible, though this case is less ideal
                ...
                # Ensure a clean state if resume fails
...
def main(cli_args):
    ...
    # Clear global states at the beginning of each main run
    extracted_loha_state_dict_global.clear()
    layer_optimization_stats_global.clear()
    params_to_seed_optimizer_global.clear()
    previously_completed_module_prefixes_global.clear()
    all_completed_module_prefixes_ever_global.clear() # This is important to clear for non-resume/non-continue cases or failed resumes
    ...
    handle_resume_or_continue_loha(
        args_global,
        extracted_loha_state_dict_global,
        params_to_seed_optimizer_global,
        previously_completed_module_prefixes_global,
        all_completed_module_prefixes_ever_global
    )
    # --- END CALL ---
    ...
    try:
        for key_name in all_candidate_keys:
            ...
            # --- Start of processing logic for the current key ---
            current_key_processed_or_skipped_good = False # Flag for this iteration
                    ...
                    try:
                        with torch.no_grad():
                            ...
                            if init_loss_c <= args_global.target_loss:
                                ...
                                skipped_good_initial_loss_count_global += 1
                                should_skip_due_to_pre_existing_good_loss = True
                                current_key_processed_or_skipped_good = True # Mark as processed for periodic save check
            ...
            if not should_skip_due_to_pre_existing_good_loss:
                ...
                if not opt_results.get('interrupted_mid_layer') and 'hada_w1_a' in opt_results :
                    ...
                    stat_entry = {
                        ...
                        "skipped_reopt_due_to_initial_good_loss": False,
            ...
            # --- Moved Periodic Save Check to here ---
            if current_key_processed_or_skipped_good: # Check if this layer's processing incremented the counter
                if args_global.save_every_n_layers > 0 and \
                   processed_layers_this_session_count_global > 0 and \
                   processed_layers_this_session_count_global % args_global.save_every_n_layers == 0 and \
                   keys_scanned_this_run_global < total_candidates_to_scan: # Avoid save if this is the very last scannable key
                    
                    periodic_save_path = generate_intermediate_filename(args_global.save_to, len(all_completed_module_prefixes_ever_global))
                    # The message uses processed_layers_this_session_count_global which is accurate for "N layers processed this session"
        ...
        # After the loop, if no interrupt, mark main loop as completed full scan
    ...
    if not save_attempted_on_interrupt:
        ...
        print(f"\n--- Overall Summary ---")
        print(f"Total unique LoHA modules in final state (processed, skipped, errored): {len(all_completed_module_prefixes_ever_global)}")
        ...
        save_to_final_name = main_loop_completed_scan_flag_global and len(all_completed_module_prefixes_ever_global) >= (total_candidates_to_scan - skipped_vae_layers_count) # Adjusted for VAE skip
        # More accurate check for full completion:
        # All candidates must either be in all_completed_module_prefixes_ever_global or be a VAE layer (which is added to all_completed... too)
        # So, len(all_completed_module_prefixes_ever_global) >= total_candidates_to_scan can be used if VAE layers are always added.
        # Let's ensure VAE layers are added to all_completed_module_prefixes_ever_global. (Done above)
        ...
        if perform_graceful_save(output_path_to_save=actual_save_path):
            if args_global.keep_n_resume_files > 0 and not is_fully_complete : # Only manage resume files if it's not the final save
                 cleanup_intermediate_files(args_global.save_to, True, args_global.keep_n_resume_files)
        ...
        if is_fully_complete and actual_save_path == args_global.save_to :
            ...
            cleanup_intermediate_files(args_global.save_to, False) # Clean all intermediates
