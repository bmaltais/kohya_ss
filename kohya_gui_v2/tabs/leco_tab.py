"""LeCo v2 tab: generic build_tab() assembly + Save/Open/Save-as/Print/
Start/Stop wiring (Move 6, checkpoint B7).

Command construction here is the fix for the confirmed LeCo crash
documented in leco_fields.py/arch-matrix-leco.md section 3: both
train_leco.py and sdxl_train_leco.py declare `--prompts_file` as
required=True, and argparse enforces it against sys.argv before
--config_file's TOML is ever read. `do_train` below always appends
`--prompts_file <value>` directly on the command line for every name in
`REQUIRED_CLI_FIELDS`, in addition to writing it into the run TOML -- the
old kohya_gui/leco_gui.py only does the latter, which is why every run
launched from it currently fails immediately with SystemExit(2).
"""
...
def leco_tab(headless: bool = False, config=None):
    ...
    with gr.Accordion("Configuration File", open=False):
        with gr.Row():
            config_file_name = gr.Textbox(
                label="Config file", value="", placeholder="./config_leco_v2.toml"
            )
    ...
    for spec in LECO_REGISTRY:
        ...
        with groups[group_key]:
            comp_dict = build_components(type(LECO_REGISTRY)([spec]), config=config)
            components.update(comp_dict)

    def apply_architecture(arch_key):
        visible = visible_groups_for(
            LECO_REGISTRY, arch_key=arch_key, training_type="leco"
        )
    ...
    ordered_names = [n for n in LECO_REGISTRY.names() if n != "architecture"]
    ...
    def do_save(arch_key, path, save_as, *component_values):
        ...
        if not path:
            return gr.Textbox()
        save_config(LECO_REGISTRY, values, path)
    ...
    def do_train(arch_key, print_only, *component_values):
        ...
        run_config = build_run_config(
            LECO_REGISTRY, values, arch_key=arch_key, training_type="leco"
        )
        ...
        toml_path = os.path.join(output_dir, f"config_leco_v2-{ts}.toml")
        ...
        script = ARCHITECTURE_SCRIPTS.get(arch_key, "train_leco.py")
        run_cmd = list(SD_SCRIPTS_BACKEND.launcher) + [
            os.path.join(SD_SCRIPTS_BACKEND.script_root, script),
        ]
        # Confirmed-bug fix (see module docstring): required=True args must
        # be on argv, not only in the TOML, since argparse's first
        # parse_args() call runs before --config_file is ever read.
