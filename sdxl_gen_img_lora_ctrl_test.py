# temporary test code for LoRA control net
...
from networks.lora_control_net import LoRAControlNet
...
class PipelineLike:
    def __init__(
        ...
        self.control_nets: List[LoRAControlNet] = []
        ...
        if self.control_nets:
            if self.control_net_enabled:
                for control_net in self.control_nets:
                    with torch.no_grad():
                        cond_embs_4d, cond_embs_3d = control_net(clip_guide_images)
...
def main(args):
    ...
    if args.control_net_models:
        for i, model_file in enumerate(args.control_net_models):
            print(f"loading control net: {model_file}")
