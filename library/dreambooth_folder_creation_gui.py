...
def dreambooth_folder_preparation(
    ...
    if not len(util_training_dir_output):
        log.info("Destination training directory is missing... can't perform the required task...")
        return
    else:
    ...
    if util_training_images_dir_input == "":
        log.info("Training images directory is missing... can't perform the required task...")
        return
    else:
    ...
    if not util_regularization_images_dir_input == "":
        ...
        if not util_regularization_images_repeat_input > 0:
            ...
            log.info(f"Copy {util_regularization_images_dir_input} to {regularization_dir}...")
            shutil.copytree(util_regularization_images_dir_input, regularization_dir)
    else:
        log.info("Regularization images directory is missing... not copying regularisation images...")
    ...
    log.info(f"Done creating kohya_ss training folder structure at {util_training_dir_output}...")
...
def gradio_dreambooth_folder_creation_tab(
    ...
    with gr.Tab("Dreambooth/LoRA Folder preparation"):
        ...
        with gr.Row():
            ...
            button_util_training_images_dir_input = gr.Button("📂", elem_id="open_folder_small", visible=(not headless))
        ...
        with gr.Row():
            ...
            button_util_training_dir_output = gr.Button("📂", elem_id="open_folder_small", visible=(not headless))
            button_util_training_dir_output.click(get_folder_path, outputs=util_training_dir_output)
