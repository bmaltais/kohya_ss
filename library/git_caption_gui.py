...
def gradio_git_caption_gui_tab(headless=False):
    with gr.Tab("GIT Captioning"):
        gr.Markdown("This utility will use GIT to caption files for each images in a folder.")
        with gr.Row():
        ...
        with gr.Row():
            max_data_loader_n_workers = gr.Number(value=2, label="Number of workers", interactive=True)
