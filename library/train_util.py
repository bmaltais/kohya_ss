...
class BucketManager:
    def __init__(
        self, no_upscale, max_reso, min_size, max_size, reso_steps
    ) -> None:
        ...
        self.buckets = (
            []
        )                     # 前処理時は (image_key, image)、学習時は image_key
    ...
    def select_bucket(self, image_width, image_height):
        ...
        if not self.no_upscale:
            ...
            if image_width * image_height > self.max_area:
                ...
                b_height_in_wr = self.round_to_steps(
                    b_width_rounded / aspect_ratio
                )
                ar_width_rounded = b_width_rounded / b_height_in_wr

                b_height_rounded = self.round_to_steps(resized_height)
                b_width_in_hr = self.round_to_steps(
                    b_height_rounded * aspect_ratio
                )
...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.width, self.height = (
            (None, None) if resolution is None else resolution
        )
    ...
    def process_caption(self, caption):
        ...
        is_drop_out = (
            self.dropout_rate > 0 and random.random() < self.dropout_rate
        )
        is_drop_out = (
            is_drop_out
        ...
        if self.enable_bucket:
            ...
            for image_info in self.image_data.values():
                ...
                (
                    ...
                    ar_error,
                ) = self.bucket_manager.select_bucket(
        ...
        for bucket_index, bucket in enumerate(self.bucket_manager.buckets):
            ...
            for batch_index in range(batch_count):
                self.buckets_indices.append(
                    BucketBatchIndex(
                        bucket_index, self.batch_size, batch_index
                    )
                )
        ...
        for info in tqdm(self.image_data.values()):
            if info.latents_npz is not None:
                ...
                if info.latents_flipped is not None:
                    info.latents_flipped = torch.FloatTensor(
            ...
            if self.flip_aug:
                image = image[
                    :, ::-1
                ].copy()     # cannot convert to Tensor without copy
                img_tensor = self.image_transforms(image)
                img_tensor = img_tensor.unsqueeze(0).to(
                    device=vae.device, dtype=vae.dtype
                )
    ...
    def load_image_with_face_info(self, image_path: str):
        ...
        if self.face_crop_aug_range is not None:
            tokens = os.path.splitext(os.path.basename(image_path))[0].split(
                '_'
            )
            if len(tokens) >= 5:
    ...
    def crop_target(self, image, face_cx, face_cy, face_w, face_h):
        ...
        min_scale = min(
            ...
            max(
                min_scale,
                self.size / (face_size * self.face_crop_aug_range[1]),
            ),
        )             # 指定した顔最小サイズ
        max_scale = min(
            1.0,
            max(
                min_scale,
                self.size / (face_size * self.face_crop_aug_range[0]),
            ),
        )             # 指定した顔最大サイズ
        ...
        for axis, (target_size, length, face_p) in enumerate(
            ...
            if self.random_crop:
                ...
                p1 = (
                    p1
                    + (random.randint(0, range) + random.randint(0, range))
                    - range
                ...
                if self.face_crop_aug_range[0] != self.face_crop_aug_range[1]:
                    if face_size > self.size // 10 and face_size >= 40:
                        p1 = p1 + random.randint(
                            -face_size // 20, +face_size // 20
                        )
    ...
    def __getitem__(self, index):
        ...
        image_index = (
            self.buckets_indices[index].batch_index * bucket_batch_size
        )
        ...
        for image_key in bucket[image_index : image_index + bucket_batch_size]:
            ...
            if image_info.latents is not None:
                ...
                (
                    ...
                    face_w,
                    face_h,
                ) = self.load_image_with_face_info(image_info.absolute_path)
                ...
                if self.enable_bucket:
                    ...
                    if face_cx > 0:                   # 顔位置情報あり
                        img = self.crop_target(
                            img, face_cx, face_cy, face_w, face_h
                        )
        ...
        if self.debug_dataset:
            example['image_keys'] = bucket[
                image_index : image_index + self.batch_size
            ]
    ...
    def __init__(
        ...
        def load_dreambooth_dir(dir):
            ...
            try:
                ...
                print(
                    f'ignore directory without repeats / 繰り返し回数のないディレクトリを無視します: {dir}'
                )
                return 0, [], []
            ...
            print(
                f'found directory {n_repeats}_{caption_by_folder} contains {len(img_paths)} image files'
            )
            ...
            for img_path in img_paths:
                ...
                captions.append(
                    caption_by_folder if cap_for_img is None else cap_for_img
                )

            self.set_tag_frequency(
                os.path.basename(dir), captions
            )         # タグ頻度を記録
        ...
        for dir in train_dirs:
            n_repeats, img_paths, captions = load_dreambooth_dir(
                os.path.join(train_data_dir, dir)
            )
            ...
            self.dataset_dirs_info[os.path.basename(dir)] = {
                'n_repeats': n_repeats,
        ...
        if reg_data_dir:
            ...
            for dir in reg_dirs:
                n_repeats, img_paths, captions = load_dreambooth_dir(
                    os.path.join(reg_data_dir, dir)
                )
                num_reg_images += n_repeats * len(img_paths)

                for img_path, caption in zip(img_paths, captions):
                    info = ImageInfo(
                        img_path, n_repeats, caption, True, img_path
                    )
                    reg_infos.append(info)

                self.reg_dataset_dirs_info[os.path.basename(dir)] = {
                    'n_repeats': n_repeats,
                    'img_count': len(img_paths),
                }
...
class FineTuningDataset(BaseDataset):
    def __init__(
        ...
        if os.path.exists(json_file_name):
            ...
            raise ValueError(
                f'no metadata / メタデータファイルがありません: {json_file_name}'
            )
        ...
        for image_key, img_md in metadata.items():
            ...
            assert (
                caption is not None and len(caption) > 0
            ), f'caption or tag is required / キャプションまたはタグは必須です:{abs_path}'

            image_info = ImageInfo(
                image_key, dataset_repeats, caption, False, abs_path
            )
            ...
            if not self.color_aug and not self.random_crop:
                ...
                (
                    ...
                    image_info.latents_npz_flipped,
                ) = self.image_key_to_npz_file(image_key)
        ...
        self.dataset_dirs_info[os.path.basename(json_file_name)] = {
        ...
        if use_npz_latents:
            ...
            for image_info in self.image_data.values():
                ...
                if self.flip_aug:
                    has_npz = (
                        has_npz and image_info.latents_npz_flipped is not None
                    )
    ...
    def image_key_to_npz_file(self, image_key):
        ...
        npz_file_flip = os.path.join(
            self.train_data_dir, image_key + '_flip.npz'
        )
...
def add_training_arguments(
    parser: argparse.ArgumentParser, support_dreambooth: bool
):
    parser.add_argument(
        '--output_dir',
        type=str,
...
def get_optimizer(args, trainable_params):
    ...
    if optimizer_type == 'AdamW8bit'.lower():
        ...
        min_lr = lr
        if (
            type(trainable_params) == list
            and type(trainable_params[0]) == dict
        ):
    ...
    optimizer_name = (
        optimizer_class.__module__ + '.' + optimizer_class.__name__
    )
    optimizer_args = ','.join(
...
def get_scheduler_fix(
    ...
    if name.startswith('adafactor'):
        ...
        return transformers.optimization.AdafactorSchedule(
...
def load_target_model(args: argparse.Namespace, weight_dtype):
    ...
    name_or_path = (
        os.readlink(name_or_path)
        if os.path.islink(name_or_path)
        else name_or_path
    )
...
def get_hidden_states(
    ...
    if args.max_token_length is not None:
        if args.v2:
            ...
            for i in range(
                1, args.max_token_length, tokenizer.model_max_length
            ):
...
def save_on_epoch_end(
    ...
    saving = (
        epoch_no % args.save_every_n_epochs == 0
        and epoch_no < num_train_epochs
    )
    if saving:
...
def save_sd_model_on_epoch_end(
    ...
    epoch_no = epoch + 1
    model_name, ckpt_name = get_epoch_ckpt_name(
        args, use_safetensors, epoch_no
    )
    ...
    if save_stable_diffusion_format:
        ...
        def remove_sd(old_epoch_no):
            _, old_ckpt_name = get_epoch_ckpt_name(
                args, use_safetensors, old_epoch_no
            )
...
def save_state_on_train_end(args: argparse.Namespace, accelerator):
    ...
    accelerator.save_state(
        os.path.join(args.output_dir, LAST_STATE_NAME.format(model_name))
    )
