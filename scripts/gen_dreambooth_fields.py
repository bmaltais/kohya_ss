"""One-off generator: build kohya_gui_v2/tabs/dreambooth_fields_generated.py
from wargame/reference/gap-analysis-dreambooth.md (port/defect/gap-candidate
per architecture) plus real argparse introspection (widget type, default,
choices) for the DreamBooth tab's 4 architectures.
...
Mirrors scripts/gen_lora_fields.py; differs only in the gap-analysis file's
section-header format (4 architecture sections here, headed
"## <label> -- `<script>.py`" instead of LoRA's "## Architecture: <key> (...)").
"""
...
# arch_key -> (module_name, gap-analysis section header prefix)
ARCH_SCRIPTS = {
    "sd15": "train_db",
    "sd2": "train_db",
    "sdxl": "sdxl_train",
...
# Which gap-analysis section each arch_key reads from. sd15/sd2 share the
# same section ("SD1.x/2.x (default)") since the matrix documents them as
# one script with universal v2/v_parameterization flags, not two scripts.
ARCH_SECTION = {
...
# Matrix-confirmed dead keys for this tab specifically (present as GUI
# widgets, never emitted to TOML for DreamBooth) -- see
# wargame/reference/arch-matrix-dreambooth.md lines 166-167, 200.
...
def main():
    ...
    lines.append("")
    lines.append("Regenerate with: uv run python scripts/gen_dreambooth_fields.py")
    lines.append(
    ...
    for name in sorted(key_meta.keys()):
        ...
        lines.append(
            ...
            f'archs={archs_repr}, training_types=frozenset({{"dreambooth"}}), group="dreambooth_generated"'
            f"{coerce_kwargs}),"
        )
    ...
    out_path = os.path.join(out_dir, "dreambooth_fields_generated.py")
