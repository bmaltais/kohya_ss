...
class FineTuningDataset(torch.utils.data.Dataset):
    ...
    def show_buckets(self):
        for i, (reso, bucket) in enumerate(
            zip(self.bucket_resos, self.buckets)
        ):
    ...
    def __getitem__(self, index):
        ...
        for image_key in bucket[image_index : image_index + self.batch_size]:
            ...
            ), f'caption or tag is required / キャプションまたはタグは必須です:{image_key}'
                ...
                if self.tokenizer.pad_token_id == self.tokenizer.eos_token_id:
                    ...
                    for i in range(
                        ...
                        ids_chunk = (
                            ...
                            input_ids[
                                i : i + self.tokenizer.model_max_length - 2
                            ],
...
def train(args):
    fine_tuning = (
        args.hypernetwork_module is None
    )            # fine tuning or hypernetwork training
    ...
    if fine_tuning:
        ...
        unet.to(
            accelerator.device
        )  # , dtype=weight_dtype)     # dtypeを指定すると学習できない
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers(
            'finetuning' if fine_tuning else 'hypernetwork'
        )
    ...
    for epoch in range(num_train_epochs):
        ...
        if args.save_every_n_epochs is not None:
            if (epoch + 1) % args.save_every_n_epochs == 0 and (
                epoch + 1
            ) < num_train_epochs:
                ...
                ckpt_file = os.path.join(
                    args.output_dir,
                    model_util.get_epoch_ckpt_name(use_safetensors, epoch + 1),
                )
                ...
                if fine_tuning:
                    if save_stable_diffusion_format:
                        ...
                        out_dir = os.path.join(
                            args.output_dir,
                            EPOCH_DIFFUSERS_DIR_NAME.format(epoch + 1),
                        )
                ...
                if args.save_state:
                    ...
                    accelerator.save_state(
                        os.path.join(
                            args.output_dir, EPOCH_STATE_NAME.format(epoch + 1)
                        )
                    )
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='num times to repeat dataset / 学習にデータセットを繰り返す回数',
    )
    parser.add_argument(
        '--output_dir',
        type=str,
    ...
    parser.add_argument(
        ...
        help='precision in saving (available in StableDiffusion checkpoint) / 保存時に精度を変更して保存する（StableDiffusion形式での保存時のみ有効）',
    )
    parser.add_argument(
        '--save_model_as',
        type=str,
